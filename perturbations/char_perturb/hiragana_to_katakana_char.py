...
"""
Hiragana to Katakana Conversion (Single Character)
Converts ONE Hiragana character in a word to Katakana.
Example: "しました" -> "しマした"
"""
...
class HiraganaToKatakana:
    """
    日本語版の文字変換攻撃クラス。
    単語に含まれる「ひらがな」のうち【一文字だけ】を「カタカナ」に変換します。
    一部の変換ミスや、意図しない文字種の混入をシミュレートします。
    """
    def __init__(self, data: Dataset, data_field: str='question', max_words: int=1, pos_tag: str=None):
        ...
        self.name: str = 'hiragana_to_katakana_char_jp'

    def execute_conversion(self, word: str) -> str:
        """
        単語内のランダムなひらがな1文字をカタカナに変換する
        """
        # ひらがなのインデックスを全て取得
        hiragana_indices = [i for i, char in enumerate(word) if re.match(r'[\u3040-\u309F]', char)]
        
        if not hiragana_indices:
            return word
            
        # 変換する位置をランダムに1つ選択
        target_idx = random.choice(hiragana_indices)
        
        # その1文字だけ変換
        target_char = word[target_idx]
        new_char = jaconv.hira2kata(target_char)
    ...
    def apply_on_sample(self, sample: Dict) -> Dict:
        ...
        # DEBUG
        print(f"\n[DEBUG-SENTENCE] --- START Attack on ID: {sample['id']} (Hira->Kata 1char) ---")
        ...
        sample[f'{self.data_field}_perturbed_H2K'] = perturbed_text
