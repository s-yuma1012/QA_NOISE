...
class SynonymReplace:
    ...
    例: "彼は速く走る" -> "彼は速く動く"
    """
    def __init__(self, data: Dataset, data_field: str='question', max_words: int=1, pos_tag: str=None, model_name: str="tohoku-nlp/bert-base-japanese-v3"):
...
if __name__ == "__main__":
    ...
    # 1. 動詞を同義語に置き換える
    # tohoku-nlp/roberta-base-japanese-v3 を使用
    replacer = SynonymReplace(data=DUMMY_DATA, data_field='question', max_words=1, pos_tag='動詞')
    
    dummy_sample = DUMMY_DATA[0].copy()
    dummy_sample['question'] = test_sentence
    
    result = replacer.apply_on_sample(dummy_sample)